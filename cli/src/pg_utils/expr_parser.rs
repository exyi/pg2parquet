...
fn parse_expr_from_tokens<'a>(mut tokens: Vec<Token<'a>>) -> Result<Expr, String> {
	...
	parser.check_end()?;
	Ok(expr)
	// let processed = pass_recursive(tokens, &|t| pass_keyword_values(pass_parse_type_names(pass_remove_ws(t))));
	// let processed = pass_recursive(processed, &pass_contract_identifiers_and_constants);
	// let processed = pass_recursive(processed, &pass_named_plans);
	// let processed = pass_recursive(processed, &pass_singular_ops);

	// let recursed: Vec<Token<'a>> = processed.into_iter().map(|t| -> Result<Token<'a>, String> {
	// 	match t {
	// 		Token::Parenthesized(inner) =>
	// 			Ok(Token::ParsedExpr(parse_expr_from_tokens(inner)?, vec![])),
	// 		t => Ok(t)
	// 	}
	// }).collect::<Result<Vec<Token<'a>>, String>>()?;

	// let recursed = pass_contract_column_access(recursed);
	// let recursed = pass_singular_ops(recursed);

	// // now we have eliminated most atomic expressions and are left with binary and unary operator resolution
	// // * seems that unary expressions are always parenthesised in EXPLAIN expressions
	// // * binaries are too, except for columns access (e.g. `(SubPlan 2).col1 + 2`), 

	// return match recursed.as_slice() {
	// 	[ Token::ParsedExpr(e, _) ] => Ok(e.clone()),

	// 	// (...)
	// 	[ Token::Parenthesized(tokens) ] => parse_expr_from_tokens(tokens.clone()),
	// 	// [ TokenTree::IdentifierString(_) | TokenTree::ConstantString(_) | TokenTree::Ident(_) | TokenTree::Number(_) | TokenTree::Punctuation(_) ] => unreachable!(),
	// 	_ => {
	// 		Ok(Expr::Unknown(format!("{}", Token::Parenthesized(recursed.clone()))))
	// 	}
	// };
}
...
impl<'a> Parser<'a> {
    pub fn new(mut tokens: Vec<Token<'a>>, recursion_limit: RecursionLimit, suppress_errors: bool) -> Self {
		tokens.reverse();
        Self { tokens_rev: tokens, ix: 0, recursion_limit, suppress_errors }
    }
	...
	pub fn check_end(&self) -> Result<(), ParseError> {
		if self.is_end() {
			Ok(())
		} else {
			Err(self.make_err(format!("Expected end of input, instead got {:?}", self.tk())))
		}
	}
			...
			match self.tk() {
				&Token::Punctuation("[") => {
					...
					loop {
						...
						if !self.is_token(&Token::Punctuation(",")) {
							break;
						}
						self.pop();
					}
				...
				Token::Parenthesized(_) => {
					...
					loop {
						...
						if !parser2.is_token(&Token::Punctuation(",")) {
							break;
						}
						parser2.pop();
					}
					...
					if !parser2.is_end() {
						return Err(parser2.make_err(format!("Expected end of argument list")));
					}
				}
...
#[test] fn test_call_index_convert() {
	assert_eq!(parse_expr("fn(1,2,'3')[123.4,12]::text::numeric"), Ok(Expr::TypeConversion(Box::new(Expr::ArrayIndex(Box::new(Expr::FunctionCall("fn".to_owned(), vec![Expr::ConstantNum("1".to_owned()), Expr::ConstantNum("2".to_owned()), Expr::ConstantStr("3".to_owned())])), vec![Expr::ConstantNum("123.4".to_owned()), Expr::ConstantNum("12".to_owned())])), ParsedType::from_name_args("numeric".to_owned(), &[], None))));
}
